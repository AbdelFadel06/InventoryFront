...
const Field = ({
  ...
  children: React.ReactNode; hint?: string; error?: string;
}) => (
  <div style={{ marginBottom: 20 }}>
    ...
    {error && <p style={{ fontSize: 11.5, color: "#EF4444", marginTop: 4, marginBottom: 0 }}>{error}</p>}
  </div>
);

const Input = ({
  value, onChange, type = "text", placeholder, min, step, disabled,
}: {
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 20, alignItems: "start" }}>
        ...
        <div style={{
          ...
          <Field label="Nom du produit" required>
            <Input value={form.name} onChange={set("name")} placeholder="Ex: Chaise ergonomique" />
          </Field>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <Field label="SKU" hint="Laissez vide pour génération auto">
              <Input value={form.sku} onChange={set("sku")} placeholder="Ex: CHR-001" />
            </Field>
            <Field label="Code-barres">
              <Input value={form.barcode} onChange={set("barcode")} placeholder="Ex: 3700123456789" />
            </Field>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            ...
            <Field label="Unité">
              <Select value={form.unit} onChange={set("unit")}>
                {[
                  ["piece", "Pièce"], ["kg", "Kilogramme"], ["g", "Gramme"],
                  ["l", "Litre"], ["ml", "Millilitre"], ["m", "Mètre"],
                  ["cm", "Centimètre"], ["box", "Boîte"], ["pack", "Pack"], ["other", "Autre"],
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <Field label="Stock minimum" hint="Alerte stock critique en dessous de ce seuil">
              <Input type="number" min="0" value={form.minimum_stock} onChange={set("minimum_stock")} />
            </Field>
            <Field label="Niveau de réapprovisionnement" hint="Alerte stock bas en dessous de ce seuil">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{
            ...
            {[
              { label: "Prix d'achat",  value: cost    > 0 ? `${cost.toLocaleString("fr-FR")} F`    : "—", color: "#64748B" },
              { label: "Prix de vente", value: selling > 0 ? `${selling.toLocaleString("fr-FR")} F` : "—", color: "#0F172A" },
              { label: "Bénéfice",      value: cost > 0 && selling > 0 ? `${profit.toLocaleString("fr-FR")} F` : "—", color: marginColor },
              { label: "Marge",         value: cost > 0 && selling > 0 ? `${margin}%` : "—", color: marginColor },
            ].map(r => (
          ...
          <div style={{
            ...
            <div style={{ fontSize: 12, fontWeight: 700, color: "#1D4ED8", marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.04em" }}>
              💡 Conseils
            </div>
            {[
              ...
              <div key={i} style={{
                fontSize: 12.5, color: "#1E40AF", marginBottom: 6,
                display: "flex", gap: 6,
              }}>
      ...
      {showCatModal && (
        <div style={{
          ...
          <div style={{
            ...
            padding: 28, width: 380, boxShadow: "0 20px 60px rgba(0,0,0,0.2)",
          }}>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>
              <Btn variant="secondary" onClick={() => { setShowCatModal(false); setNewCatName(""); }}>
                Annuler
              </Btn>
              <Btn onClick={handleCreateCategory} disabled={creatingCat || !newCatName.trim()}>
